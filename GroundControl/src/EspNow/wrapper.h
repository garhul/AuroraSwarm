...
//   bool addDevice(const Node* device, bool asBroker = false);
...
//   bool updateNode(uint8_t index, const Node* device);
//   bool callDevice(uint8_t macAddr[6], uint8_t* payload, uint8_t len);
//   bool sendToNode(const Node& device, const uint8_t* msg, uint8_t len);
